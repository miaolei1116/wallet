...
import {PrivateKey} from "gxbjs/es";
...
class LastBackupDate extends Component {
    render() {
        ...
        let backup_time = backup_date ?
            <h4 className="facolor-error" style={{fontSize:"0.8rem"}}><Translate content="wallet.last_backup" /> <FormattedDate value={backup_date}/></h4>:
        ...
        if( backup_date ) {
            needs_backup = last_modified.getTime() > backup_date.getTime() ?
                <h4 style={{fontSize:"0.8rem"}} className="facolor-error"><Translate content="wallet.need_backup" /></h4>:
                <h4 style={{fontSize:"0.8rem"}} className="success"><Translate content="wallet.noneed_backup" /></h4>
        }
        return <span>
            <div className="grid-block" style={{paddingTop:"0.5rem",marginLeft:"3rem"}}>
                <div className="grid-content">{needs_backup}</div>   
                <div className="grid-content" style={{}}>{backup_time}</div>
            </div>
        </span>
    }
}
...
class Upload extends Component {
    ...
    render() {
        let resetButton = (
            <div style={{paddingTop: 20,paddingLeft:"2rem"}}>
                <div
        ...
        if(
            ...
            return <div>{this.props.children}<div style={{position:"relative",top:"-3.6rem",left:"6rem"}}>{resetButton}</div></div>;
        ...
        return (
            <div>
                <input ref="file_input" accept=".bin" type="file" id="backup_input_file" 
    ...
    render() {
        return <span>
            ...
            {this.props.backup.last_modified ?
                <div style={{marginBottom:"1.6rem"}}>{new Date(this.props.backup.last_modified).toLocaleString()}</div> : null }
            <br/>
        </span>
    }
}
    ...
    render() {
        ...
        <form onSubmit={this.onPassword.bind(this)}>
            ...
            <div className="grid-block">
                <div className="grid-block medium-3" style={{marginLeft:"1rem",paddingTop:"1rem"}}>请输入密码</div>
...
export {BackupCreate, BackupVerify, BackupRestore, Restore, NewWalletName,
    Download, Create, Upload, NameSizeModified, DecryptBackup, Sha1, LastBackupDate};
